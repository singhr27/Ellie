...
if not API_KEY:
    raise RuntimeError("Set ASSEMBLYAI_KEY in your environment before running.")
...
def on_message(ws, message):
    try:
        ...
        if msg_type == "Begin":
            ...
            if end:
                final_text=data.get("utterance") or transcript
                #store in queue for main thread processing
                final_transcript_queue.put(final_text)
...
def run():
    ...
    try:
        ...
        while ws_thread.is_alive():
            try:
                user_text = final_transcript_queue.get(timeout=0.3)
                print("\nUser said:", user_text)
                ...
                reply = client.responses.create(
                    model="gpt-5-nano",
                    input= [
        {"role": "system", "content": "You are an AI companion for youth. Always be concise and kind. Respond in 1-2 short sentences, each answer should be <=20 words. Be resourceful and efficient"},
        {"role": "user", "content": user_text}
    ]
                )
                # Play the response using Murf TTS
                play_streaming_audio(reply.output_text)
                print("\n🤖Companion:", reply.output_text)
